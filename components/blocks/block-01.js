(function (blocks, editor, element, components, data) {
    ...
    const blockStructure = (posts) => (
        posts.map(post =>
            el('article', { key: post.id, className: "unit-01", style: { fontSize: "0.7em" } },
                el('div', { className: "column" },
                    el('div', { className: "featured-image-container" },
                        el('img', { src: post.fimg_url, alt: post.title.rendered, style: { width: '100%' } })
                    )
                ),
                el('div', { className: "column va-center" },
                    el('h2', { style: { fontSize: "2rem" } }, post.title.rendered),
    ...
    const icon = el('img', {src:'./icons/block01.svg'})
    ...
    blocks.registerBlockType('litci/block-01', {
        ...
        edit: withSelect(function (select) {
            ...
            var posts = useSelect((select) => {
                ...
                var query = {
                    per_page: 1, // Número de posts a serem exibidos
