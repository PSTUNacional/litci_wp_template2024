(function (blocks, editor, element, components, data) {
  ...
  const blockStructure = (posts, videoId, videoTitle) =>
    el(
      "div",
      {},
      el(
        ...
        el(
          ...
          el(
            "div",
            { className: "info", style: { fontSize: "0.7em" } },
        ...
        el(
          ...
          el(
            ...
            el("h3", { style: { color: "#000" } }, videoTitle)
          )
        )
      )
    );
  ...
  const icon = el("img", {
    src: "../wp-content/themes/litci/components/blocks/icons/block06.svg",
  });

  blocks.registerBlockType("litci/block-12", {
    title: "LIT-Bloco 12",
    icon: icon,
    ...
    edit: withSelect(function (select) {
      // Busca todas as caetgorias
      var categories = select("core").getEntityRecords("taxonomy", "category", {
        per_page: -1,
      });
      var categoryOptions = [];
      ...
      var onVideoIdChange = function (newId) {
        props.setAttributes({ videoId: newId });
      };

      var onVideoTitleChange = function (newTitle) {
        props.setAttributes({ videoTitle: newTitle });
      };
      ...
      var posts = useSelect(
        (select) => {
          ...
          var query = {
            ...
            orderby:
              attributes.customIds.length > 0
                ? "include"
                : attributes.sortOption === "menu_order"
                ? "menu_order"
                : "date",
            order: "desc",
          };
          ...
          if (
            attributes.blockCategories.length > 0 &&
            attributes.customIds.length == 0
          ) {
        ...
        [
          ...
          attributes.blockCategories,
          attributes.customIds,
        ]
      );
      ...
      return el(
        ...
        el(
          ...
          el(
            PanelBody,
            { title: "Filtro automático", initialOpen: false },
            el(SelectControl, {
              label: "Ordenação",
            ...
            el(
              "fieldset",
              { className: "category-multi-select-container" },
          ...
          el(
            PanelBody,
            { title: "Filtro manual", initialOpen: false },
            el(TextControl, {
              label: "IDs dos posts",
          ...
          el(
            PanelBody,
            { title: "Vídeo personalizado", initialOpen: true },
            el(TextControl, {
